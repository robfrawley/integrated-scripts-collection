...
#
# perform bpytop install operation
#

function runner_installer() {
  local script_path
  local logger_path
  ...
  "${script_path}/bpytop-remove.bash"

  logger_path="${script_path}/.bpytop-install.log"
  ...
  printf -- 'Working to create virtual environment for "bpytop" using python (version %s) interpreter ... ' "$(python3 --version 2>&1 | cut -d' ' -f2)"
  ...
  printf -- 'Working to install "bpytop (all)" package using pip (version %s) ... ' "$(pip --version | cut -d' ' -f2)"

  if pip install 'bpytop[all]' &> "${logger_path}"; then
    printf -- '[success]\n'
  else
    printf -- '[failure]\nLogging file located at "%s" ... Exiting prematurely!\n' "${logger_path}"
    exit 255
  ...
  printf -- 'Exiting installation script for "bpytop" without error (executable at "%s/bin/bpytop") ...\n' "${script_path}"
